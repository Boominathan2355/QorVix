#!/usr/bin/env python3
"""Generate the CUDA embedding model implementation file."""
import os

OUT = os.path.join(os.path.dirname(__file__), '..', 'cuda', 'src', 'embedding_model.cu')

code = r'''// CUDA BERT-family embedding engine (Phase 11c) - GPU implementation of IEmbeddingEngine.
...
class CudaEmbedImpl : public EmbeddingModel {
 public:
  ...
  bool embedWith(const std::vector<int>& tokens, int pooling, bool normalize,
    ...
    if (tab_.embdNorm)
      layernormK<<<n, kEmbThreads>>>(scratch_[0], scratch_[0], tab_.embnNorm, tab_.embdNormB, d, cfg_.normEps);
