      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="space-y-1">
          ...
          <h2 className="text-2xl font-bold text-foreground tracking-tight flex items-center gap-2">
            <AudioIcon size={24} className="text-purple-500" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-5 space-y-5">
          <Card glass className="p-6 space-y-5">
            ...
            <div className="relative rounded-2xl overflow-hidden border border-border bg-card h-36 flex flex-col items-center justify-center shadow-inner">
              <canvas
                ref={canvasRef}
                width={380}
                height={144}
              ...
              {!isRecording && !audioUrl && (
                <div className="absolute inset-0 flex flex-col items-center justify-center text-muted-foreground text-xs font-mono space-y-1">
                  <AudioIcon size={24} className="text-muted-foreground/60" />
            ...
            <div className="flex items-center justify-center gap-3">
              ...
              <label className="inline-flex items-center justify-center font-medium transition-all duration-150 h-9 px-4 rounded-xl text-sm bg-secondary hover:bg-muted text-foreground border border-border cursor-pointer gap-2 shadow-xs">
                <UploadIcon size={16} />
            ...
            {audioUrl && (
              <div className="p-3.5 rounded-xl bg-secondary border border-border space-y-2">
                <div className="flex items-center justify-between text-xs font-mono text-muted-foreground">
                  <span>RECORDED AUDIO</span>
                  <span className="text-teal-600 dark:text-teal-400 font-semibold">Ready to process</span>
                </div>
            ...
            {task === 'transcribe' && (
              <div className="space-y-1.5">
                ...
                <select
                  ...
                  className="w-full bg-background border border-border rounded-xl px-3.5 py-2 text-xs font-mono text-foreground focus:outline-none focus:border-teal-500/50 cursor-pointer"
                >
            ...
            <div className="flex-1 flex flex-col justify-between">
              {result ? (
                <div className="space-y-4">
                  <div className="p-4 rounded-2xl bg-secondary border border-border text-foreground text-sm leading-relaxed select-text">
                    {result.text}
                  </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="max-h-60 overflow-y-auto space-y-1.5 pr-1">
                        {result.segments.map((seg) => (
                          <div
                            key={seg.id}
                            className="p-2.5 rounded-xl bg-card border border-border flex items-start gap-3 text-xs"
                          >
                            <span className="font-mono text-teal-600 dark:text-teal-400 font-semibold bg-teal-500/10 px-2 py-0.5 rounded border border-teal-500/20 shrink-0">
                ...
                <div className="h-full flex flex-col items-center justify-center text-center space-y-2 py-16 text-muted-foreground">
                  <AudioIcon size={36} className="text-muted-foreground/50" />
