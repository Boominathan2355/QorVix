  ...
  return (
    <div className="p-6 md:p-8 max-w-7xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div className="space-y-1">
          ...
          <h2 className="text-2xl font-bold text-slate-100 tracking-tight flex items-center gap-2">
            <ModelsIcon size={24} className="text-teal-400" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {models.map((model) => (
          <Card key={model.id} glass hover className="p-6 space-y-5 border-slate-800">
            <div className="flex items-start justify-between">
              <div className="p-3 rounded-2xl bg-teal-950/60 border border-teal-800/40 text-teal-400">
                <ModelsIcon size={24} />
              </div>
            ...
            <div className="space-y-1.5">
              <h3 className="font-bold text-base text-slate-100 truncate" title={model.id}>
                {model.id}
              </h3>
            ...
            <div className="p-4 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2 text-xs font-mono">
              <div className="flex items-center justify-between text-slate-400">
                ...
                <span className="text-teal-300 font-bold">{model.backend || 'CUDA / Native'}</span>
              </div>
              <div className="flex items-center justify-between text-slate-400">
                <span className="flex items-center gap-1.5"><HardDriveIcon size={14} /> Quantization</span>
                <span className="text-slate-200">{model.quantization || 'Q4_K_M (4-bit block)'}</span>
              </div>
              <div className="flex items-center justify-between text-slate-400">
                <span className="flex items-center gap-1.5"><LayersIcon size={14} /> Context Window</span>
                <span className="text-slate-200">{model.context_length || '4096 / 8192'} tokens</span>
              </div>
            </div>
          </Card>
        ))}
